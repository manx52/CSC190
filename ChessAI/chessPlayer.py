...
for i in range(0,nl,1):
        ...
        if IsOnBoard(ll,board,pos):
            if IsCollision(ll, board, pos) == 1:
                accum += [ll]
                break
            elif IsCollision(ll, board, pos) == 2:
                break
            else:
                accum += [ll]
    ...
    for i in range(0,nl,1):
        ul += 9
        if IsOnBoard(ul,board,pos):
            if IsCollision(ul, board, pos) == 1:
                accum += [ul]
                break
...
def GetPawnMoves(pos, board):
    ...
    if board[pos] // 10 == 1:
        pp = pos + 8
        if IsOnBoard(pp, board, pos) and IsFriendly(pp,board,pos) and  not(IsCollision(pp,board,pos) == 1):
            tmp += [pp]
        ppr = pos + 7
...
def BestMove(board, player, tree):
    ...
    if tree.store[0] == 0:
        ...
        if len(tmp) > 1:
            ...
            for i in tree.store[1]:
               if i.score == mx and len(tree.store[1]) == len(tmp) :
                    if board[i.store[0][0]] // player == 1 and board[i.store[0][0]] - player == 0:
                        tmp2 += [i.store[0]]
               '''elif i.score == mx:
                    if board[i.store[0][0]] // player == 1 and board[i.store[0][0]] - player != 5:
 tmp2 += [i.store[0]]'''



            if tmp2 == []:
                move = random.choice(tmp)
            else:
                move = random.choice(tmp2)
...
def chessPlayer(board, player):
    ...
    tmp = AddDepth(board, player, root,player)
